...
const PettyCashPage = () => {
  ...
  const handleTabChange = (_, newVal) => {
    setActiveTab(newVal);
    if (
      newVal === (isAdminOrSuper ? 2 : -1) ||
      (isAdminOrSuper && newVal === 2)
    ) {
      fetchSummary();
    }
  };
  ...
  const handleTransferSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        date:
          transferForm.date instanceof Date
            ? transferForm.date.toISOString()
            : transferForm.date,
        description:
          transferForm.description ||
          `Petty cash transfer from ${transferForm.sourceAccount}`,
        remarks: transferForm.remarks,
        transactionRef: transferForm.transactionRef,
        paymentMode:
          transferForm.sourceAccount === "Bank" ? "BankTransfer" : "Cash",
      };
  ...
  const handleExpenseSubmit = async () => {
    ...
    if (!branchId) {
      setExpenseError("No branch assigned.");
      return;
    }
    ...
    try {
      const payload = {
        ...
        date:
          expenseForm.date instanceof Date
            ? expenseForm.date.toISOString()
            : expenseForm.date,
        ...
        paymentMonth: expenseForm.paymentMonth
          ? new Date(expenseForm.paymentMonth).toLocaleString("default", {
              month: "long",
              year: "numeric",
            })
          : undefined,
      ...
      setExpenseError(
        e.response?.data?.message || "Failed to add expense. Please try again."
      );
    } finally {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      await daybookService.update(editId, {
        ...
        paymentMonth:
          editForm.paymentMonth instanceof Date
            ? editForm.paymentMonth.toISOString()
            : editForm.paymentMonth || undefined,
      });
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box>
        <PageHeader
          title="Petty Cash"
          subtitle="Manage petty cash transfers and expenses"
        >
          {!isAdminOrSuper && (
            <Button
              variant="contained"
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 3 }}>
            {isAdminOrSuper ? (
              ...
              <Typography variant="subtitle1" fontWeight="medium">
                Branch:{" "}
                {branches.find((b) => b._id === dashBranch)?.name || "—"}
              </Typography>
            )}
            <Button
              variant="outlined"
                    ...
                    {receivedEntries.length === 0 ? (
                      ...
                      receivedEntries.map((row) => (
                        <TableRow key={row._id} hover>
                          ...
                          {isAdminOrSuper && !dashBranch && (
                            <TableCell>{row.branchId?.name || "-"}</TableCell>
                          )}
                          ...
                          <TableCell>
                            {row.attachments?.length > 0 ? (
                              <Button
                                ...
                                onClick={() =>
                                  window.open(
                                    daybookService.getAttachment(
                                      row._id,
                                      row.attachments[0]._id
                                    ),
                                    "_blank"
                                  )
                                }
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth required>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          handleTransferChange("branchId", e.target.value)
                        }
                        label="Branch"
                      >
                        {branches.map((b) => (
                  ...
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth required>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          handleTransferChange("sourceAccount", e.target.value)
                        }
                  ...
                  <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      onChange={(e) =>
                        handleTransferChange("amount", e.target.value)
                      }
                      inputProps={{ min: 0 }}
                    />
                  </Grid>
                  ...
                  <Grid item xs={12} sm={6}>
                    <TextField
                      ...
                      onChange={(e) =>
                        handleTransferChange("transactionRef", e.target.value)
                      }
                    />
                  </Grid>
                  ...
                  <Grid item xs={12}>
                    <TextField
                      ...
                      onChange={(e) =>
                        handleTransferChange("description", e.target.value)
                      }
                    />
                  </Grid>
                  ...
                  <Grid item xs={12}>
                    <TextField
                      ...
                      onChange={(e) =>
                        handleTransferChange("remarks", e.target.value)
                      }
                    />
                  </Grid>
                </Grid>
                          ...
                          <TableRow key={row.branchId} hover>
                            ...
                            <TableCell align="right">
                              <Typography color="success.main">
                                {formatCurrency(row.totalGiven)}
                              </Typography>
                            </TableCell>
                            <TableCell align="right">
                              <Typography color="error.main">
                                {formatCurrency(row.totalUtilized)}
                              </Typography>
                            </TableCell>
                            <TableCell align="right">
                              <Chip
                        ...
                        <TableRow sx={{ bgcolor: "grey.50" }}>
                          ...
                          <TableCell align="right">
                            <Typography fontWeight="bold">
                              {formatCurrency(
                                summaryData.reduce((s, r) => s + r.remaining, 0)
                              )}
                            </Typography>
                          </TableCell>
                        </TableRow>
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              <Grid item xs={6}>
                <DatePicker
                  ...
                  onChange={(val) =>
                    setEditForm({ ...editForm, paymentMonth: val })
                  }
                  views={["month", "year"]}
                  format="MMMM yyyy"
                  slotProps={{ textField: { fullWidth: true } }}
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, transactionRef: e.target.value })
                  }
                />
              </Grid>
              <Grid item xs={12}>
              ...
              <Grid item xs={12}>
                  ...
                  <Box sx={{ mb: 1 }}>
                    {editAttachments.map((att) => (
                      <Button
                        ...
                        onClick={() =>
                          window.open(
                            daybookService.getAttachment(editId, att._id),
                            "_blank"
                          )
                        }
                      >
                ...
                {editForm.attachmentFile && (
                  <Button
                    ...
                    sx={{ ml: 1 }}
                    onClick={() =>
                      setEditForm({ ...editForm, attachmentFile: null })
                    }
                  >
                    Remove
                  </Button>
            ...
            <Grid container spacing={2} sx={{ mt: 0 }}>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  onChange={(e) =>
                    handleExpenseChange("transactionRef", e.target.value)
                  }
                />
              </Grid>
              ...
              <Grid item xs={12}>
                <TextField
                  ...
                  onChange={(e) =>
                    handleExpenseChange("remarks", e.target.value)
                  }
                />
              </Grid>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  onChange={(e) =>
                    handleExpenseChange("paidTo", e.target.value)
                  }
                  placeholder="Recipient name"
                />
              </Grid>
              ...
              <Grid item xs={12}>
                <Button variant="outlined" component="label" size="small">
                  {expenseForm.attachmentFile
                    ? expenseForm.attachmentFile.name
                    : "Attach Receipt / Bill"}
                  <input
                    type="file"
                    hidden
                ...
                {expenseForm.attachmentFile && (
                  <Button
                    ...
                    color="error"
                    sx={{ ml: 1 }}
                    onClick={() => handleExpenseChange("attachmentFile", null)}
                  >
                    Remove
                  </Button>
                )}
